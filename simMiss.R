...
sim_df <- function(n, muX, sigmaX, alpha0, alphaX, alphaA,
  ...
  ## Missingness
  M <- ifelse(X < 0.4, 0.1, 0.4)
  M <- matrix(
    rbinom(length(X), size = 1, prob = as.vector(M)),
    nrow = nrow(X), ncol = ncol(X))
  X[M == 1] <- NA
  ## Mean imputation
  meanX <- numeric(0)
  for (i in 1:ncol(X)) {
    meanX <- mean(X[,i], na.rm = T)
    X[is.na(X[, i]), i] <- meanX
  }
  #apply(M, 1, max) %>% mean()
...
## Set Parameters
n <- 1000; muX = c(2,-1,1); sigmaX = diag(c(0.3, 0.3, 0.3), nrow = 3, ncol = 3)
...
getwd()
write.csv(rel_eff_var_n1, "/home/mcolonva/AUC_HS/relative_efficienty_miss4.csv")
write.csv(bias_n1, "/home/mcolonva/AUC_HS/bias_miss4.csv")
